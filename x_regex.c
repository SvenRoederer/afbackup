...
#include <version.h>
  ...
  static char * fileversion = "$RCSfile: x_regex.c,v $ $Source: /home/alb/afbackup/afbackup-3.5.8.12/RCS/x_regex.c,v $ $Id: x_regex.c,v 1.5 2012/11/01 09:52:59 alb Exp alb $ " PACKAGE " " VERSION_STRING;
...
re_find_match_once(UChar * pattern, UChar * string, Int32 * s, Int32 * e)
{
  ...
  if(re_compile_pattern(pattern, strlen(pattern), &re_buf))
    return(errno = ENOMEM);
  /* once upon a time re_compile_pattern reused re_buf, when not
   * freed by regfree. So it was possible to do some optimization.
   * These days are gone *sigh* */

  res = re_find_match(&re_buf, string, s, e);
